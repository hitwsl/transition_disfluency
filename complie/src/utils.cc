...
void load_pretrained_word_embedding(const std::string& embedding_file,
  ...
  if (!in) {
    _WARN << "Failed to open word embedding file, is there something wrong?";
  }
  std::string line;
  std::getline(in, line);
...
float k_log_k(float i) {
  float low = 1., high = 2.;
  while (true) {
    if (high * log(high) >= i) { break; }
  ...
  while (low < high && high - low > 1e-1) {
    mid = (low + high) * 0.5;
    if (mid * log(mid) < i) { low = mid; }
    else { high = mid; }
  }
  return mid;
}
