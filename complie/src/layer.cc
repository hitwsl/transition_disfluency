...
StaticInputLayer::StaticInputLayer(cnn::Model *model,
                                   unsigned gen_method, unsigned dim_gen,unsigned size_word, unsigned dim_word,
        ...
        use_word(true), use_postag(true), use_gen(true), use_pretrained_word(true) {
...
cnn::expr::Expression StaticInputLayer::add_input(cnn::ComputationGraph *hg,
                                                  unsigned wid, unsigned pid, unsigned pre_wid,const cnn::expr::Expression &gen_fea) {
    ...
    if(use_gen){
        ...
        //cnn::expr::Expression t = cnn::expr::const_lookup(*hg, p_t, pre_wid);
        expr = cnn::expr::affine_transform({expr, g2l, gen_fea});
    }
...
//    bw_lstm.get_h(cnn::RNNPointer(n_items - index)).back());
//}
//
//
...
BidirectionalLSTMLayer::Output BidirectionalLSTMLayer::get_output_origin(cnn::ComputationGraph *hg,
                                                                         int index) {
    return std::make_pair(
            fw_lstm.get_h(cnn::RNNPointer(index)).back(),
            bw_lstm.get_h(cnn::RNNPointer(n_items - index)).back());
}
...
void BidirectionalLSTMLayer::get_outputs_origin(cnn::ComputationGraph *hg,
                                                std::vector<BidirectionalLSTMLayer::Output> &outputs) {
    outputs.resize(n_items + 1);
    for (unsigned i = 0; i <= n_items; ++i) {
        outputs[i] = get_output_origin(hg, i);
    }
}
...
//  }
    if (gen_method  == 0) {
        p_G = model->add_parameters({dim_output, dim_gen_fea});
        use_gen = true;
    }
...
void StaticInputBidirectionalLSTM::get_inputs(cnn::ComputationGraph *hg,
                                              ...
                                              std::vector<cnn::expr::Expression> &exprs,const std::vector<std::vector<cnn::real>>& current_gen_feature,const unsigned dim_gen_fea) {
    ...
    for (unsigned i = 0; i < len; ++i) {
        ...
        exprs[i] = input_layer.add_input(hg, wid, pid, pre_wid,cnn::expr::input(*hg, {dim_gen_fea}, &current_gen_feature[i]));
    }
}
