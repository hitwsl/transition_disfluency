...
struct BidirectionalLSTMLayer {
  ...
  Output get_output_origin(cnn::ComputationGraph* hg, int index);
  void get_outputs(cnn::ComputationGraph* hg, std::vector<Output>& outputs);
  void get_outputs_origin(cnn::ComputationGraph* hg, std::vector<Output>& outputs);//including the first
  void set_dropout(float& rate);
  void disable_dropout();
};
...
struct SoftmaxLayer {
  ...
  SoftmaxLayer(cnn::Model* model, unsigned dim_input, unsigned dim_gen_fea, unsigned dim_output, unsigned gen_method);
  cnn::expr::Expression get_output(cnn::ComputationGraph* hg,
    const cnn::expr::Expression& expr,const cnn::expr::Expression& gen_fea);
};
...
struct StaticInputBidirectionalLSTM {
  ...
  StaticInputBidirectionalLSTM(cnn::Model* model,
    ...
    input_layer(model, gen_method,gen_dim,size_word, dim_word,
